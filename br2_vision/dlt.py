...
class DLT:
    ...
    def inverse_map(self, x, y, z):
        ...
        for k, param in self.camera_param.items():
            u = (param[0] * x + param[1] * y + param[2] * z + param[3]) / (
                param[8] * x + param[9] * y + param[10] * z + 1
            )
            v = (param[4] * x + param[5] * y + param[6] * z + param[7]) / (
                param[8] * x + param[9] * y + param[10] * z + 1
            )
            uvs[k] = (u, v)
        return uvs
    ...
    def save(self, force=False):
        ...
        if self.calibration_path is None:
            ...
            return 0
        save_path_information = os.path.join(
            self.calibration_path, "camera_information.json"
        )
    ...
    def add_camera(self, camera_id: int, calibration_type=16, tag="", verbose=False):
        ...
        if verbose:
            print(
                "New Camera Added - id: {},  type: {}, desc: {}".format(
                    camera_id, calibration_type, tag
                )
            )
    ...
    def finalize(self, verbose=False):
        ...
        for k, camera in self.camera.items():
            ...
            if verbose:
                print(
                    "camera {} calibration error (in pixel) : {}+={}".format(
                        k, error, error_vector.std()
                    )
                )
        self._finalize = True
    ...
    class Camera:
        ...
        def calculate_parameters(self, verbose=False):
            ...
            assert (
                self.validity()
            ), "Not enough reference points are given to calibrate the camera"
            ...
            if self.type > 11:
                ...
                param_ext = np.zeros(self.type)
                param_ext[:11] = param[:11]
                ...
                while delta > self.iterative_threshold:
                    ...
                    L_ext[:, :11] = np.array(L)
                    g_ext = g.copy()
                    for idx, (u, v, x, y, z, xi, eta, r_sqr) in enumerate(
                        self.references
                    ):
                        R = (
                            param_ext[8] * x + param_ext[9] * y + param_ext[10] * z + 1
                        )  # eqn 14
                        print(xi, eta, r_sqr)
                    ...
                    if verbose:
                        print(
                            "Iteration {}: delta={}, target threshold={}, error={}".format(
                                iteration, delta, self.iterative_threshold, error
                            )
                        )
...
class DLT2D:
    ...
    def map(self, u, v):
        ...
        """
        raise NotImplementedError(
            "Co-planar mapping is not possible with single camera."
        )
    ...
    def inverse_map(self, x, y):
        ...
        param = self.param
        u = int(
            (param[0] * x + param[1] * y + param[2]) / (param[6] * x + param[7] * y + 1)
        )
        v = int(
            (param[3] * x + param[4] * y + param[5]) / (param[6] * x + param[7] * y + 1)
        )
        return u, v
    ...
    def save(self, force=False):
        ...
        # Save camera parameters in NPZ
        np.savez(self.save_path, param=self.param)
