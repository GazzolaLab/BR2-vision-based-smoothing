...
class ForwardBackwardSmooth(Algorithm):
        ...
        for index, s in enumerate(data.s_position):
            for s_index in range(self.s_position.shape[0]):
                if s_index == 0:
                    ...
                    s_range_plus = (
                        self.s_position[s_index + 1] - self.s_position[s_index]
                    ) / 2
                    s_range_minus = (
                        self.s_position[s_index] - self.s_position[s_index - 1]
                    ) / 2
                ...
                if (s_plus - s) * (s - s_minus) >= -1e-5:
                    ...
                    self.s_position_jump_index.append(s_index)
                    break
        self.s_position_jump_index = np.array(self.s_position_jump_index, dtype=int)
            ...
            for index, s in enumerate(data.s_director):
                for s_index in range(self.s_director.shape[0]):
                    if s_index == 0:
                        ...
                        s_range_plus = (
                            self.s_director[s_index + 1] - self.s_director[s_index]
                        ) / 2
                        s_range_minus = (
                            self.s_director[s_index] - self.s_director[s_index - 1]
                        ) / 2
    ...
    @njit(cache=True)
    def energy_cost(
        dl, shear, kappa, rest_shear, rest_kappa, shear_matrix, bend_matrix
    ):
        cost = 0
        for n in range(dl.shape[0]):
            for i in range(3):
                cost += (
                    (shear[i, n] - rest_shear[i, n]) ** 2
                    * shear_matrix[i, i, n]
                    * dl[i]
                )
    ...
    @staticmethod
    @njit(cache=True)
    def argument_cost(dl, argument_rotational, argument_translational, argument_weight):
        cost = 0
    ...
    def data_position_cost(
        ...
        for index, n in enumerate(s_position_jump_index):
            cost += data_deviation_weight_cost_position * position_cost(
                position[:, n], data_position[:, index]
            )
        return cost
    ...
    def data_director_cost(
        ...
        for n, index in enumerate(s_director_jump_index):
            if index != -1:
                cost += data_deviation_weight_cost_director * director_cost(
                    director[:, :, n], data_director[:, :, index]
                )
        # for index, n in enumerate(s_director_jump_index):
        #    cost += data_deviation_weight_cost_director * director_cost(director[:, :, n], data_director[:, :, index])
        return cost

    def update_strain_and_forward_path(
        self,
    ):
        self.update_argument(
            self.argument_translational,
            self.argument_rotational,
            self.costate_translational,
            self.costate_rotational,
            self.config.argument_weight,
    ...
    def update_without_director_jump(
        self,
    ):
    ...
    def update_argument(
        ...
        for n in range(argument_rotational.shape[1]):
            ...
            argument_translational[:, n] += step_size * (
                costate_translational[:, n]
                - argument_weight * argument_translational[:, n]
            )
        argument_translational[:, -1] += step_size * (
            costate_translational[:, -1]
            - argument_weight * argument_translational[:, -1]
        )
    ...
    def costate_backward_path(
        ...
        for i in range(3):
            costate_rotational[i, -1] = (
                -(
                    -internal_couple[i, -1]
                    + bend_matrix[i, i, -1] * (kappa[i, -1] - rest_kappa[i, -1])
                )
                * dl[-1]
            )
            costate_translational[i, -1] = (
                -(
                    -internal_force[i, -1]
                    + shear_matrix[i, i, -1] * (shear[i, -1] - rest_shear[i, -1])
                )
                * dl[-1]
            )
        ...
        for n in range(blocksize - 2):
            for i in range(3):
                costate_rotational[i, -1 - n - 1] = (
                    costate_rotational[i, -1 - n]
                    - (
                        -internal_couple[i, -1 - n - 1]
                        + bend_matrix[i, i, -1 - n - 1]
                        * (kappa[i, -1 - n - 1] - rest_kappa[i, -1 - n - 1])
                    )
                    * dl[-1 - n - 1]
                )
                costate_translational[i, -1 - n - 1] = (
                    costate_translational[i, -1 - n]
                    - (
                        -internal_force[i, -1 - n - 1]
                        + shear_matrix[i, i, -1 - n - 1]
                        * (shear[i, -1 - n - 1] - rest_shear[i, -1 - n - 1])
                    )
                    * dl[-1 - n - 1]
                )

        for i in range(3):
            costate_translational[i, 0] = (
                costate_translational[i, 1]
                - (
                    -internal_force[i, 0]
                    + shear_matrix[i, i, 0] * (shear[i, 0] - rest_shear[i, 0])
                )
                * dl[0]
            )
    ...
    def backward_path_without_director_jump(
        ...
        internal_force_lab_frame = calculate_internal_force_with_jumping_condition(
            position,
            director,
            internal_force,
            s_position_jump_index,
        ...
        internal_couple_lab_frame = calculate_internal_couple_without_jumping_condition(
            dl,
            director,
            shear,
    ...
    def backward_path_with_director_jump(
        ...
        # calculate internal couple
        internal_couple_lab_frame = calculate_internal_couple_with_jumping_condition(
            dl,
            director,
            shear,
            internal_force_lab_frame,
...
def calculate_internal_force_with_jumping_condition(
    ...
    precalc_values = np.zeros((3, s_position_jump_index.shape[0]))
    for index, i in enumerate(s_position_jump_index):
        precalc_values[:, index] -= data_deviation_weight_cost_position * (
            position[:, i] - data_position[:, index]
        )

    # Reduced gather
    prev_i = 0
    for index, i in enumerate(s_position_jump_index):
        internal_force_lab_frame[:, prev_i:i] = precalc_values[:, index : index + 1]
        prev_i = i

    # calculate internal force in lab frame
    # for i in range(blocksize-1):
    #    # this is the jumping condition
    #    if s_position_jump_index2[-1-i] != -1:
    #        internal_force_lab_frame2[:, -1-i] -= data_deviation_weight_cost_position * (
    #            position[:, -1-i] - data_position[:, s_position_jump_index2[-1-i]]
    #        )
    #    internal_force_lab_frame2[:, -1-i-1] = internal_force_lab_frame2[:, -1-i].copy()
...
def calculate_internal_couple_with_jumping_condition(
    ...
    # Pre-calculation
    # precalc_values = np.zeros((3, s_director_jump_index.shape[0]))
    # for index, i in enumerate(s_director_jump_index):
    #    precalc_values[:,index] += data_deviation_weight_cost_director * (
    #            difference_between_directors(
    #                director[:, :, i], data_director[:, :, index]
    #            )
    #        )
    ...
    # for i in range(blocksize-1):
    #    internal_couple_lab_frame[:, -1-i-1] -= (ms[:, -1-i]+ms[:, -1-i-1])*dl[-1-i]/2
    ...
    for i in range(blocksize - 1):
        ...
        if s_director_jump_index[-1 - i] != -1:
            internal_couple_lab_frame[
                ...
                difference_between_directors(
                    director[:, :, -1 - i],
                    data_director[:, :, s_director_jump_index[-1 - i]],
                )
            )

        internal_couple_lab_frame[:, -1 - i - 1] = (
            internal_couple_lab_frame[:, -1 - i]
            - (ms[:, -1 - i] + ms[:, -1 - i - 1]) * dl[-1 - i] / 2
        )
...
def couple_in_lab_transform_to_material(internal_couple_lab_frame, director):
    ...
    for i in range(blocksize - 1):
        middle_couple = (
            internal_couple_lab_frame[:, i] + internal_couple_lab_frame[:, i + 1]
        ) / 2
...
def _vec(skew_matrix):
    return np.array([-skew_matrix[1, 2], skew_matrix[0, 2], -skew_matrix[0, 1]])
