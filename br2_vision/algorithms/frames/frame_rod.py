...
class RodFrame(Frame):
    def __init__(self, figure_name, folder_name, fig_dict, gs_dict, **kwargs):
        Frame.__init__(
            self, figure_name=figure_name, folder_name=folder_name, fig_dict=fig_dict
        )
    ...
    def reset(
        ...
        self.gs = gridspec.GridSpec(figure=self.fig, **self.gs_dict)
    ...
    def plot_rod2d(self, position, director, radius, color=None):
        ...
        self.ax_rod.plot(
            p_up[0], p_up[1], color=self.rod_color if color is None else color
        )
        self.ax_rod.plot(
            p_down[0], p_down[1], color=self.rod_color if color is None else color
        )
        self.ax_rod.plot(
            p_left[0],
            p_left[1],
    ...
    def plot_loads(self, external_force, external_couple):
        for index_i in range(3):
            ...
            self.axes_external_force[index_i].plot(
                self.s_external_force, external_force[index_i], color=self.rod_color
            )
            ...
            self.axes_external_couple[index_i].plot(
                self.s_external_couple, external_couple[index_i], color=self.rod_color
            )
    ...
    def set_ax_strains_lim(
        self,
        axes_shear_lim=None,
        axes_curvature_lim=None,
    ):
        if axes_shear_lim is None:
            axes_shear_lim = [[-0.11, 0.11], [-0.11, 0.11], [-0.1, 2.1]]
        ...
        for index_i in range(3):
            ...
            curvature_log = np.floor(
                np.log10(axes_curvature_lim[index_i][1] - curvature_mean)
            )
            ...
            self.axes_shear[index_i].ticklabel_format(
                axis="y", scilimits=(shear_log, shear_log), useOffset=shear_mean
            )
            self.axes_curvature[index_i].ticklabel_format(
                axis="y",
                scilimits=(curvature_log, curvature_log),
                useOffset=curvature_mean,
            )

    def set_ax_loads_lim(
        self, axes_external_force_lim=None, axes_external_couple_lim=None
    ):
        if axes_external_force_lim is None:
            axes_external_force_lim = [[-0.11, 0.11], [-0.11, 0.11], [-0.11, 0.11]]
        ...
        for index_i in range(3):
            ...
            self.axes_external_couple[index_i].set_ylim(
                axes_external_couple_lim[index_i]
            )
            self.axes_external_force[index_i].ticklabel_format(
                axis="y", scilimits=(-1, -1)
            )
            self.axes_external_couple[index_i].ticklabel_format(
                axis="y", scilimits=(-3, -3)
            )

    def set_labels(self, time=None):
        if time is not None:
            self.ax_rod.set_title(
                "time={:.2f} [sec]".format(time), fontsize=self.fontsize
            )
        for index_i in range(3):
            self.axes_curvature[index_i].set_ylabel(
                "    d$_{}$".format(index_i + 1), fontsize=self.fontsize, rotation=0
            )
        ...
        self.axes_shear[2].text(
            0, position, "stretch", fontsize=self.fontsize, ha="left", va="top"
        )
        ...
        ylim_mean = np.average(ylim)
        position = 0.9 * (ylim[1] - ylim_mean) + ylim_mean
        self.axes_curvature[2].text(
            1, position, "twist", fontsize=self.fontsize, ha="right", va="top"
        )
