...
def get_center_and_normal(labelled_points: dict, n_ring: int):
    ...
    for i in range(n_ring):
        ...
        assert (
            len(A) > 1
        ), f"Each plane must have more than 1 reference point to interpolate properly. {point_group}"
        A = np.array(A)
        y = np.ones([A.shape[0]])
        ...
        if i == 0:  # First vector should be in positive y-direction
            ...
            if np.linalg.norm(tangent - prev_tangent) > np.linalg.norm(
        ...
        plane_center = np.array(
            [_RADIUS / np.cos(np.pi / 6), 0.0, 0.0]
        )  # Point we want to find in local frame
        plane_space = []
        dlt_space = []
...
def append_all_marker_points(labelled_points: dict, num_ring: int):
    ...
    for ring_idx in range(1, num_ring + 1):
        available_point_labels = [
            key for key in labelled_points.keys() if f"R{ring_idx}" in key
        ]
...
if __name__ == "__main__":
    from br2_vision.data.marker_positions import MarkerPositions

    NUM_RING = 9
    NUM_POINT = 9
    RING_CHAR = "R"
    all_tags = []
    spacing = [0.0465, 0.04125, 0.038, 0.0355, 0.0350, 0.0485, 0.030, 0.0320, 0.0345]
    coords = {}
    for ring_id in range(1, NUM_RING + 1):
        for point_id in range(NUM_POINT):
            ...
            if ring_id == 1:
                print(tag, coord)
                coords[f"R{point_id}"] = coord

    mp = MarkerPositions(marker_center_offset=spacing, marker_positions=coords)
    print(mp)
    mp.to_yaml("test.yaml")
