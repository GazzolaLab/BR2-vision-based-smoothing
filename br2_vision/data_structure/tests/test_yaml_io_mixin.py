...
class MockClass(DataclassYamlSaveLoadMixin):
    ...
    dictionary_type: Dict[str, Tuple[float, float, float]] = field(
        default_factory=lambda: {"key1": (0.0, 0.0, 0.0), "key2": (0.0, 0.0, 0.0)}
    )
    dictionary_type2: Dict[str, List[float]] = field(
        default_factory=lambda: {"key1": [0.0, 0.0, 0.0], "key2": [0.0, 0.0, 0.0]}
    )
