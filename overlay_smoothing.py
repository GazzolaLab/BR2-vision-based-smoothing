...
SMOOTHED_FILE_PATH = os.path.join(PATH, SMOOTHING_DIR, 'smoothing-run-{}.pickle'.format(RUNID))
...
CENTER_OFFSET = np.array([0.06718874,0.08053348,0.23180589])   # base for 090221 single BR2
...
interp_position = interpolate.interp1d(smoothed_time, smoothed_position, axis=0)(exp_time)
interp_director = interpolate.interp1d(smoothed_time, smoothed_director, axis=0)(exp_time)
...
interp_director = np.pad(interp_director, ((0,0),(0,0),(0,0),(1,0)), mode='linear_ramp')
...
for position, director in zip(interp_position, interp_director):
    uvs_director_d1.append(dlt.inverse_map(*(position+vector_length*director[0,:,:])))
    uvs_director_d2.append(dlt.inverse_map(*(position+vector_length*director[1,:,:])))
    uvs_director_d3.append(dlt.inverse_map(*(position+vector_length*director[2,:,:])))
...
for camid in tqdm(range(1, num_camera+1), position=0):
    ...
    #print(f'{video_size=}')
    #print(f'{frame.shape=}')
    writer = cv2.VideoWriter(OVERLAY_SMOOTHING_VIDEO_PATH.format(camid, RUNID),
    ...
    while cap.isOpened() and ret:
        ...
        idx = -1
        for u, v, d1u, d1v, d2u, d2v, d3u, d3v in zip(*uvs_position[num_frame][camid],
            ...
            if idx % skip_every != 0:
                continue
            u = int(u); v = int(v)
            d1u = int(d1u); d1v = int(d1v)
            d2u = int(d2u); d2v = int(d2v)
            d3u = int(d3u); d3v = int(d3v)
            layer_centerline = cv2.circle(layer_centerline, (u,v), 9, color_centerline, -1)
            layer_d1lines = cv2.line(layer_d1lines, (u,v), (d1u, d1v), color_d1, 4) # d1
            layer_d2lines = cv2.line(layer_d2lines, (u,v), (d2u, d2v), color_d2, 4) # d2
            layer_d3lines = cv2.line(layer_d3lines, (u,v), (d3u, d3v), color_d3, 4) # d3
