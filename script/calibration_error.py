...
calibration_images = glob.glob(os.path.join(IMAGE_PATH, '{}-*-{}-*[0-9].png'.format(CAMERA_INDICATOR, PURPOSE_INDICATOR)))
...
for (camera_id, x_id), path in reference_image_paths.items():
    save_path_points=os.path.join(IMAGE_PATH, 'cam-{}-calibration-{}-refpoints.npz'.format(camera_id, x_id))
...
for k, uvs in point_collection.items():
    ...
    error_vector.append([true_x, true_y, true_z, dlt_x-true_x, dlt_y-true_y, dlt_z-true_z])
