...
contents = html.Div([
    ...
    html.Div(id='tabs-content-camera-data', children=[
        dcc.Graph(
            id=f'graph-x',
        ),
        dcc.Graph(
...
banner = html.Div(
    ...
    className="banner",
    children=[
        html.Img(src="https://github.com/skim0119/BR2-simulator/blob/main/docs/_static/assets/logo_v1.png", style=BANNER_STYLE),
    ],
...
@callback([Output('graph-x', 'figure'),
           Output('graph-y', 'figure')],
           Input('tabs-camera-data', 'value'))
...
def get_camera_data(cid) -> pd.DataFrame:
    ...
    with TrackingData.load(path=datapath) as dataset:
        ...
        num_frames = get_video_frame_count(config["PATHS"]["footage_video_path"].format(tag, cid, run_id))
