...
class Frame(RodFrame, DirectorFrame):
    def __init__(self, figure_name, folder_name, fig_dict, gs_dict, rod_color, **kwargs):
        RodFrame.__init__(self, figure_name, folder_name, fig_dict, gs_dict, rod_color=rod_color, **kwargs)
...
def create_movie(file_name, delta_s_position):
    ...
    for k in tqdm(range(len(time))):
        ...
        rod_ax.view_init(elev=10., azim=60)
        ...
        if k != 0:
            frame.plot_data(
                position=rotate_frame(orientation, position=data.position[data_index[k]]),
                director=rotate_frame(orientation, director=data.director[data_index[k]])
            )
        ...
        frame.set_ax_rod_lim(z_lim=[-1.1, 0.1], x_lim=[-0.55, 0.55], y_lim=[-0.55, 0.55])
...
if __name__ == "__main__":
    ...
    parser.add_argument('problem', metavar='subproblem number', type=str, nargs=1,
