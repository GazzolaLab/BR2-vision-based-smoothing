...
def process_data(file_name, delta_s_position):
    ...
    for k in range(data.time.shape[0]-1):
        ...
        delta_position = np.abs(smoothed_data['position'][-1]-algo.position).max() * 1000
        ...
        num_iter = len(cost)
        print(f'time={k} : {runtime=:0.2e} sec, {delta_cost=:0.2e}, {delta_position=:0.2f} mm, {delta_director=:0.2e}, {delta_shear=:0.2e}, {delta_kappa=:0.2e}, {num_iter=}')
